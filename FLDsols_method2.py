...
#    divergence.direction = -1  # velocity gradient becomes negative (v goes up as we go in, we're diverging for sure)
    ...
    result = solve_ivp(dr_fld, rspan, (T0,v0), method='RK45', dense_output=True, events=(causality,divergence), rtol=1e-6)    
...
def MakeWindFLD(params, logMdot, Verbose=0): # this would go into the full MakeWind later
    ...
    rs = W.rSonic(Ts)
    res1 = W.outerIntegration(returnResult=True)[0]
    r1 = res1.t
    T1,phi1 = res1.sol(r1)
...
def get_map(logMDOT,Edot):
    ...
    Tsvals = np.linspace(7,7.3,n)
    vinfvals = np.linspace(8,9,n)
